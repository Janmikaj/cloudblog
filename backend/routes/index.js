...
router.post('/register', async (req, res) => {
  try {
    ...
    await newUser.save();
    res.status(201).json({ message: 'User registered successfully', role: newUser.role, username: newUser.username, email: newUser.email });
  } catch (error) {
    res.status(500).json({ error: 'Server error' });
  }
});
...
// POST /blogs - Create blog (intentionally no auth middleware for vulnerability testing)
